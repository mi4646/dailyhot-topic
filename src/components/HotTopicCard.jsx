...
const HotTopicCard = ({ sourceData, openModal, handleRetry, error = null }) => {
  const handleClick = () => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col h-full transform transition-transform duration-300 hover:scale-105 hover:shadow-xl relative overflow-hidden">
      ...
      >
        {error ? (
          ...
          <ErrorCard
            message={error}
            onRetry={() => handleRetry(sourceData.source)}
          />
        ) : (
          // 正常内容区域
      ...
      <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
        ...
        <button className="px-3 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 rounded-full hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors duration-200 flex items-center">
